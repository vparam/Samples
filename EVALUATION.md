...
product, podcast, video) on the **local backend** (the default).

The prototype now ships with two pluggable retrieval backends, switched
by `MJS_SEARCH_BACKEND`:

| Backend | What runs | When to use |
|---|---|---|
| `local` (default) | scikit-learn TF-IDF + `rank-bm25`, score-based fusion, intent-gated recency boost | Offline / local demos / CI |
| `azure` | Azure AI Search hybrid (BM25 + `text-embedding-3-large` vectors via integrated vectorisation) → semantic cross-encoder re-rank → `recency-boost` scoring profile | Production; needs `AZURE_SEARCH_*` env vars |

The numbers below are from the local backend because that is what runs
in this sandbox without credentials. The Azure backend is exercised by
13 offline tests using `httpx.MockTransport` (`tests/test_azure_backend.py`):
search request shape, no-results threshold against
`@search.rerankerScore`, push/delete batches, factory wiring, and
ingestion replication after commit. A reviewer who points the
prototype at a real AI Search resource gets the production retrieval
path with no code changes — see *Switching to real Azure AI Search* in
`prototype/README.md`.

Score (local) = score-based hybrid fusion of BM25 and TF-IDF cosine,
after the bounded recency multiplier. Recency boost is gated by intent:
full 1.3× cap when the query carries words like *recent / latest / new*,
otherwise a tighter 1.02× cap so freshness does not flip ranking on
close calls. The Azure backend mirrors this contract via the
`recency-boost` scoring profile (architecture §5), applied at the
service.
...
|---|---|---|
| **Retrieval quality** | Local backend: A **5/5**, B **7/7**, C **5/5**, D 3/5 closed (2 are by-design no-synthesis limits, brief-aligned), E **8/8**. Recency boost works *and* is correctly bounded so it does not override relevance on vague queries. Azure backend: same response envelope, validated by mocked-transport tests covering hybrid request shape, semantic re-rank, recency scoring profile, and pure-recency listing. | **Maximised on the local stack; Azure path wired and offline-tested.** Score-based fusion + intent-gated recency + small synonym table + attack-pattern guard is the right local shape. Flipping to Azure trades the synonym table for `text-embedding-3-large` semantic similarity and the score-based fusion for the L2 cross-encoder semantic ranker — the same architecture, different backend. |
| **Grounding discipline** | E: **8/8 no_results — 0 fabricated answers.** Every returned URL across all 30 queries is a real entry in the SQLite index (verified by the test suite). The constraint is structural on both backends: there is no LLM in the read path; the local backend can only return SQLite rows; AI Search's semantic re-ranker is a transformer cross-encoder, not a generative model. Attack-pattern detection runs client-side on both backends, before any AI Search call, so adversarial queries never reach the service. | **Maximised. Architectural, not prompted.** The prototype cannot be tricked into making things up — on either backend it can only retrieve indexed rows or return the no-results state. |
| **Architectural judgment** | The prototype ships **both** the local stand-in and a real Azure AI Search backend behind the same `get_index()` factory, switched by `MJS_SEARCH_BACKEND`. Every substitution is documented and recoverable: TF-IDF↔`text-embedding-3-large`, sklearn↔AI Search, SQLite↔Postgres. Every gap closed in this round (synonym table, intent gates, attack patterns, pluggable backend) was added at the right layer; nothing patched into a fragile prompt; the read-path grounding contract was never disturbed. | Trade-offs deliberate; production swap is one env var away. |
| **Microsoft alignment** | Architecture specifies AI Search + AOAI + Entra + App Insights + Container Apps Jobs + Key Vault + paired-region replicas. `samples/storage/ai-search-index.json` is a real index definition the prototype can PUT directly via `azure_search.create_or_update_index()`; `samples/storage/postgres-schema.sql` is the production-shaped DDL. SSO mocked with the same token shape so the swap to real Entra is mechanical. The Azure backend uses integrated AOAI vectorisation (no embedding code in our process), AI Search's native semantic ranker, and the documented `recency-boost` scoring profile (§5). | **Aligned and exercised end-to-end.** |
| **Code quality** | 10 backend modules with single responsibilities; **75 tests** organised by requirement category, including 13 offline Azure-backend tests using `httpx.MockTransport`. New functionality (synonym expansion, intent gates, attack patterns, date-sort listing, pluggable Azure backend, post-commit replication) added without disturbing the read-path grounding contract. | Reasonable to read and extend. |
| **Communication** | Trade-offs and divergences listed explicitly in `prototype/README.md` (including a "Switching to real Azure AI Search" section with env-var table and one-time setup recipe); this evaluation enumerates concrete behaviour rather than hand-waving; every closed gap names what was wrong, what changed, and why. | Honest. |

---

## What I would do differently with more time

The previous version of this list led with *wire real Azure AI Search*.
That has now landed (commit `b6f3666`): the Azure backend ships behind
a `get_index()` factory, exercised end-to-end by 13 offline tests using
`httpx.MockTransport`, and turns on with three env vars. What's
remaining, in priority order:

1. **Run the Azure backend against a real AI Search resource and
   re-run this evaluation on it.** Everything is in place; what's
   missing is the credentials. Two things I expect to see:

   - **Group D vocabulary mismatch (D1 *bioplastics*) goes from "closed
     by hand-curated synonym table" to "closed by `text-embedding-3-large`
     semantic similarity"** — robust to abbreviations the table doesn't
     know. The synonym table can shrink to a smoke-test list.
   - **The vague-intent ranking on A5 gets sharper** because the L2
     cross-encoder semantic re-ranker scores documents on actual
     semantic similarity to the query, not just lexical overlap. The
     local backend's intent-gated recency cap is the right shape but
     the cross-encoder is a bigger lever.

   I would also re-tune `MJS_AZURE_RERANKER_THRESHOLD` against the
   golden eval set; the default 1.5 is conservative.
...
2. **Filterable facets in the UI.** The architecture's index already
   ...
   synthesis. Closes Group D's first by-design limit.
...
4. **Promote attack detection from regex to a small adversarial
...
5. **Production WebSub subscriber-registration loop.** The push
   ingestion for YouTube and podcasts is wired and tested end-to-end
   (`tests/test_websub.py`), but the loop that POSTs to the hub's
   `/subscribe` endpoint with our callback URL and periodically
   re-subscribes on lease expiry is stubbed. ~80 lines of code; runs
   on the same scheduler.

6. **Stronger evaluation harness.** The current 30-query demo is
   ...
   delta?" so future changes — including local↔Azure backend swaps —
   have an evidence-based merge bar.

7. **Production hardening per architecture §9** — paired-region AI
   Search replica, Key Vault + managed-identity auth (replacing
   `AZURE_SEARCH_KEY` env var), private endpoints, row-level audit on
   admin metadata edits, embedding-spend dashboard in App Insights,
   alerting on ingestion lag > 10 min and zero-result-rate spikes.

8. **Backend-aware integration tests in CI.** The 75 tests today all
   target the local backend (with mocked Azure transport for the 13
   Azure tests). A staging environment with a real AI Search instance,
   triggered nightly, would catch index-schema drift and AI Search API
   version changes the mocks cannot.
